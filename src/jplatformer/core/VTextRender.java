...
public final class VTextRender {
    ...
    private static ArrayList<VTextRender> textRender = new ArrayList<VTextRender>();

    public static VTextRender getTextRender(String name, int style, int size) {
        return getTextRender(new Font(name, style, size));
    }

    public static VTextRender getTextRender(Font f) {
        for (VTextRender text : textRender) {
            if (text.f.getName().equals(f.getName()) && text.f.getStyle() == f.getStyle() && text.f.getSize() == f.getSize()) {
                return text;
            }
        }
        return new VTextRender(f);
    }

    private VTextRender(Font f) {
        this.f = f;
        init();
        textRender.add(this);
    }
